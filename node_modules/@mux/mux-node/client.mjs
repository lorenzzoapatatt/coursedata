...
import { RobotsPreview } from "./resources/robots-preview/robots-preview.mjs";
...
export class Mux {
     ...
     */
    constructor({ baseURL = readEnv('MUX_BASE_URL'), tokenId = readEnv('MUX_TOKEN_ID') ?? null, tokenSecret = readEnv('MUX_TOKEN_SECRET') ?? null, webhookSecret = readEnv('MUX_WEBHOOK_SECRET') ?? null, jwtSigningKey = readEnv('MUX_SIGNING_KEY') ?? null, jwtPrivateKey = readEnv('MUX_PRIVATE_KEY') ?? null, authorizationToken = readEnv('MUX_AUTHORIZATION_TOKEN') ?? null, ...opts } = {}) {
        ...
        this.logLevel =
            parseLogLevel(options.logLevel, 'ClientOptions.logLevel', this) ??
                parseLogLevel(readEnv('MUX_LOG'), "process.env['MUX_LOG']", this) ??
                defaultLogLevel;
        this.fetchOptions = options.fetchOptions;
        this.maxRetries = options.maxRetries ?? 2;
        this.fetch = options.fetch ?? Shims.getDefaultFetch();
        __classPrivateFieldSet(this, _Mux_encoder, Opts.FallbackEncoder, "f");
        const customHeadersEnv = readEnv('MUX_CUSTOM_HEADERS');
        if (customHeadersEnv) {
            const parsed = {};
    ...
    buildURL(path, query, defaultBaseURL) {
        const baseURL = (!__classPrivateFieldGet(this, _Mux_instances, "m", _Mux_baseURLOverridden).call(this) && defaultBaseURL) || this.baseURL;
    ...
    buildBody({ options: { body, headers: rawHeaders } }) {
        ...
        ArrayBuffer.isView(body) ||
            ...
            return __classPrivateFieldGet(this, _Mux_encoder, "f").call(this, { body, headers });
        }
    }
}
